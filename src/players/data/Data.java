...
public class Data {
	...
	private static volatile boolean tcpReceiverRunning = true;
	private static ServerSocket serverSocket;
	private static volatile boolean udpReceiverRunning = true;
	private static MulticastSocket socket;
	
	public static String ipFinder() {
        ...
        for (String address : testAddresses) {
            try {
                // Adresse IP externe à tester pour déterminer la bonne adresse ip
                ...
                try (Socket socket = new Socket()) {
                	socket.connect(new InetSocketAddress(externalAddress, 53), 5000);
                ...
                System.err.println("Erreur : Adresse inconnue " + address);
            } catch (SocketTimeoutException e) {
                System.err.println("Erreur : Timeout lors de la connexion à " + address);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }

        // Retourne null si aucune ip n'a été trouvée
        return null;
    }
	...
	public static String writeMessage(String key, LinkedHashMap<String, String> attributs) {
	    ...
	    for (String i : attributs.keySet()) {
	        String attrValue = attributs.get(i);
	        // Échapper les caractères spéciaux dans la valeur de l'attribut
	        attrValue = attrValue.replace("&", "&amp;")
	                             .replace("\"", "&quot;")
	                             .replace("<", "&lt;")
	                             .replace(">", "&gt;")
	                             .replace("'", "&apos;");
	        message.append(" ").append(i).append("=\"").append(attrValue).append("\"");
	    }
 ...
 // File d'attente pour stocker les messages reçus
    ...
    public static void udpReceiver(String localInterface,Consumer<String> messageConsumer) {
        // Validation des entrées
        if (localInterface == null || localInterface.isEmpty()) {
            throw new IllegalArgumentException("L'interface réseau ne doit pas être null ou vide.");
        }
        ...
        new Thread(() -> {
            ...
            try {
                ...
                while (udpReceiverRunning) {
                    ...
                    // Ajouter le message au consumer
                    messageConsumer.accept(received);
                }
    ...
    public static String getMessage() {
        if (!messageQueue.isEmpty()) {
            return messageQueue.poll(); // Retirer et retourner le message, si disponible
        }
        return null; // Aucun message à traiter
    }
    

    public static void tcpReceiver(int serverPort, BiConsumer<String, String> messageConsumer) {
        new Thread(() -> {
            try  {
            	serverSocket = new ServerSocket(serverPort);
                while (tcpReceiverRunning) {
                    try (Socket clientSocket = serverSocket.accept();
                         BufferedReader reader = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()))) {
                        
                        String receivedMessage = reader.readLine();
                        String clientIP = clientSocket.getInetAddress().getHostAddress(); // Récupérer l'adresse IP
                        messageConsumer.accept(receivedMessage, clientIP); // Passer le message et l'IP
                    } catch (IOException e) {
                        System.err.println(e.getMessage());
                    }
                }
            } catch (BindException e) {
                System.err.println("Erreur : le port " + serverPort + " est déjà utilisé ou inaccessible. tcp");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }).start();
    ...
    // Méthode pour arrêter le thread tcpReceiver
    public static void stopTcpReceiver() {
        tcpReceiverRunning = false; // Arrêter la boucle
        try {
            if (serverSocket != null && !serverSocket.isClosed()) {
                serverSocket.close(); // Fermer le ServerSocket pour arrêter proprement
            }
        } catch (IOException e) {
            System.err.println("Erreur lors de la fermeture du tcpReceiver : " + e.getMessage());
        }
    }
 // Méthode pour arrêter le thread tcpReceiver
    public static void stateUdpRunning() {
        udpReceiverRunning = true;
    }

 // Méthode pour arrêter le thread tcpReceiver
    public static void stopUdpReceiver() {
        udpReceiverRunning = false; // Arrêter la boucle
        if (socket != null && !socket.isClosed()) {
		    socket.close();
		}
    }
 // Méthode pour arrêter le thread tcpReceiver
    public static void stateTcpRunning() {
        tcpReceiverRunning = true;
    }
    ...
    public static Object[] readMessage(String message) {
        ...
        // Expression régulière pour extraire les paires nom=valeur en tenant compte des guillemets
        Matcher attrMatcher = Pattern.compile("(\\S+?)\\s*=\\s*(\"[^\"]*\"|\\S+)").matcher(content);
        ...
        while (attrMatcher.find()) {
            ...
            // Déséchapper les caractères spéciaux dans la valeur de l'attribut
            attrValue = attrValue.replace("&quot;", "\"")
                                 .replace("&apos;", "'")
                                 .replace("&gt;", ">")
                                 .replace("&lt;", "<")
